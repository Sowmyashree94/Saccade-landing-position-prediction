...
data_out_dir = "/home/niteesh/Documents/uni/HCI/Saarland/Npy_files_noMask/"
subject_count = 1

min_saccade_samples = 13 #With this parameter, only saccades longer than it are recorded
num_samples_train = 15 #For every saccade, only first num_samples_train are stored in npy file for training
...
#    
        ...
        if velocity_filtered[i] >= Vd:
            ...
            for j in range(i-1,i-num_samples_anchor,-1):
                ...
                if (velocity_filtered[j]==Va):
                    ...
                    pass
            if found == True:           
                not_glissade = False #if length between anchor and end point is less than 15ms, dont consider
                for k in range(i+1,len(velocity_filtered)):
                    if (velocity_filtered[k]<Vf):
                        if (k-temp_indices[1]) > offset_15ms_glissades:
    ...
    #            print (temp_indices)
                if not_glissade:
                    ...
                    diff = gaze_coordinates_2d[temp_indices[2]+offset] - gaze_coordinates_2d[temp_indices[1]+offset]
                    dist = np.linalg.norm(diff)
                    saccade_amp = dist * deg_per_px
                    if saccade_amp > threshold_amp:
                        try:
...
user = 0
#
indices_range = [int(SaccadeIndices_allUsrs[user][num,0]),int(SaccadeIndices_allUsrs[user][num,2])]
#
#fig = plt.figure()
